    ...
    <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
      <div className="rounded border border-gray-200 bg-white p-5">
        ...
        <div className="space-y-3">
          ...
          <NumberField label="送料" value={form.shippingCost} onChange={(e) => setForm({ ...form, shippingCost: Number(e.target.value) || 0 })} />
          <NumberField label="ロス率" step="0.01" value={form.lossRate} onChange={(e) => setForm({ ...form, lossRate: Number(e.target.value) || 0 })} />
              ...
              {materials.map((row) => (
                <tr key={row.id} className="border-b last:border-b-0">
                  ...
                  <td className="px-4 py-3 text-right">
                    <button className="text-red-700" onClick={() => remove(row.id)}>削除</button>
                  </td>
                </tr>
              ))}
            </tbody>
